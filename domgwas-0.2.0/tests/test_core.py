"""Automated tests for the domgwas core."""
import numpy as np
import pandas as pd
import pytest
...
from domgwas.testutils import simulate, write_plink
...
def sim(tmp_path_factory):
    ...
    res = domgwas.run_gwas(prefix, meta["pheno"], model="add-dom", verbose=False)
...
def test_per_trait_missingness_and_covariates(sim):
    ...
    result = domgwas.run_gwas(prefix, pheno, covar=covar, chroms=["1"], verbose=False)
...
def test_binary_trait_is_rejected(sim):
    ...
    with pytest.raises(ValueError, match="binary phenotypes are not supported"):
        domgwas.run_gwas(prefix, binary, chroms=["1"], verbose=False)
...
def test_additive_model_runs(sim):
    prefix, meta, _ = sim
    res = domgwas.run_gwas(prefix, meta["pheno"], model="additive", verbose=False)
